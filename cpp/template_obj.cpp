...
Template::Template(std::string name, std::vector<StringKey> keys, std::string definition)
{
	this->_name = name;
	this->_keys = keys;
...
std::map<std::string, std::string> Template::getFields(std::string path)
{
	...
	std::cout << "" << std::endl;
	std::cout << static_parse  << std::endl;
...
std::vector<std::string> Template::splitPath(const std::string& path) {
	char delimiter = '\\';
...
std::string Template::joinListWithSeparator(std::vector<std::string> list, char separator) {
	std::string result = "";
