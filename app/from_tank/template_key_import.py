...
class TemplateKey(object):
    ...
    def __init__(
        ...
        if self.shotgun_field_name and not self.shotgun_entity_type:
            raise ValueError("%s: SG field requires a SG entity be set." % self)
    ...
    def str_from_value(self, value=None, ignore_type=False):
        ...
        :raises: :class:`ValueError` if value is not valid for the key.
        """
        if value is None:
...
class StringKey(TemplateKey):
    ...
    def __init__(
        ...
        if self._subset_format and sys.version_info < (2, 6):
            raise ValueError("Subset formatting in template keys require python 2.6+!")
...
class IntegerKey(TemplateKey):
    ...
    def _init_format_spec(self, name, format_spec):
        ...
        :raises ValueError: Raised when the parameter is not a string that maching a %d format
                           option.
        """
        ...
        if not isinstance(format_spec, six.string_types):
            ...
            raise ValueError(msg % (name, format_spec))

        if len(format_spec) == 0:
            raise ValueError("format_spec can't be empty.")
    ...
    def _init_strict_matching(self, name, strict_matching):
        ...
        :raises ValueError: Raised when the parameter is not a boolean.
        """
        ...
        if not (strict_matching is None or isinstance(strict_matching, bool)):
            ...
            raise ValueError(msg % (name, str(strict_matching)))
        ...
        if self._format_spec is None and strict_matching is not None:
            raise ValueError("strict_matching can't be set if there is no format_spec")
