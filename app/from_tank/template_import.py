...
from template_key_import import TemplateKey, make_keys
from template_path_parser_import import TemplatePathParser
...
class Template(object):
    ...
    def get_fields(self, input_path, skip_keys=None):
        ...
        if fields is None:
            raise ValueError("Template %s: %s" % (str(self), path_parser.last_error))
...
class TemplatePath(Template):
    ...
    :class:`Template` representing a complete path on disk. The template definition is multi-platform
     you can pass it per-os roots given by a separate :meth:`root_path`.
    """
    ...
    def _apply_fields(
        ...
        if platform is None:
            ...
            platform = six.normalize_platform(platform)
            ...
            platform_root_path = self._per_platform_roots.get(six.PLATEFORM)
...
def _conform_template_data(template_data, template_name):
    ...
    if "definition" not in template_data:
        raise ValueError("Template %s missing definition." % template_name)
...
def read_templates(pipeline_configuration):
    ...
    keys = make_keys(get_data_section("keys"))
...
def make_template_paths(data, keys, all_per_platform_roots, default_root=None):
    ...
    for template_name, template_data in templates_data.items():
        ...
        root_path = all_per_platform_roots.get(root_name, {}).get(six.PLATEFORM)
...
def make_template_strings(data, keys, template_paths):
    ...
    for template_name, template_data in templates_data.items():
        ...
        if validator_name and not validator:
            ...
            raise ValueError(msg % (template_name, validator_name))
...
if __name__ == '__main__':
    import yaml
    from pipiconfiguration_import import PipelineConfiguration

    # with open(r"D:\Desk\python\Tank\config\template.yml", "r") as stream:
    #     _templates = yaml.safe_load(stream)

    pipe = PipelineConfiguration("D:\Desk\python\Tank")

    templates = read_templates(pipe)
